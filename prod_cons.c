...
typedef struct {
    ...
    int numHilo;       // Índice del hilo en el bucle de iniciación
} Datos;

// Mutexes, variables de condición y barrera como variables globales
...
void insertarItem(char item, Datos *datos) {
    ...
    buffer.fin = (buffer.fin +1) % N; //Lo incrementamos de forma modular
}
...
void *productor(void *arg) {
    ...
    while (item != '*') {
        ...
        if((item>=48 && item<=57) || (item>=65 && item<=90) || (item>=97 && item<=122) || item=='*'){
            ...
            // Despierta al consumidor y libera el acceso a región crítica
...
void *consumidor(void *arg) {
     ...
     */
    while (contProd != numProd) {
        // Simula tiempo de consumo
        sleep(rand()%(datosCons->T));     
        ...
        while(buffer.numElem == 0){
            ...
            // Para evitar que hilos consumidores se queden bloqueados aquí cuando no haya más items
        ...
        // Despierta al productor y libera el acceso a región crítica
        pthread_cond_signal(&condp);
        pthread_mutex_unlock(&mutex);
    }

    // Despierta al posible consumidor bloqueado
    pthread_cond_signal(&condc);  

    // Se cierra el fichero de escritura, se libera la memoria de datosCons y se finaliza el hilo
    fclose(fp);
    free(datosCons);
    pthread_exit(NULL);
}
...
int main(int argc, char** argv) {
    ...
    int numCons;
    if(argc != 3){
        perror("Error: no ha insertado el número requerido de argumentos. Formato aceptado: ./exe <num_prod> <num_cons> ");
        exit(EXIT_FAILURE);
    }else{
        if((numProd=atoi(argv[1]))==0){
            if(strcmp(argv[1], "0")==0){
                perror("Error: el número de productores a crear debe de ser mayor que 0. Formato aceptado: ./exe <num_prod> <num_cons> ");
            }else{
                perror("Error: el primer parámetro no es un número entero. Formato aceptado: ./exe <num_prod> <num_cons> ");
            }
            exit(EXIT_FAILURE);
        }
        if((numCons=atoi(argv[2]))==0){
            if(strcmp(argv[2], "0")==0){
                perror("Error: el número de consumidores a crear debe de ser mayor que 0. Formato aceptado: ./exe <num_prod> <num_cons> ");
            }else{
                perror("Error: el segundo parámetro no es un número entero. Formato aceptado: ./exe <num_prod> <num_cons> ");
            }
            exit(EXIT_FAILURE);
        }
        if(numProd<0 || numCons<0){
            perror("Error: el número de productores y consumidores a crear debe de ser mayor que 0. Formato aceptado: ./exe <num_prod> ");
            exit(EXIT_FAILURE);
        }
    }

    // Paso 2: Se inicializan variables necesarias, la barrera para poder emplearla entre hilos distintos
    // y los mutexes y variables de condición que se emplearán entre hilos y entre productor y consumidor
    contProd= 0;
    // Inicializar la cola vacía
    buffer.inicio = 0;
    buffer.fin= 0;
    buffer.numElem= 0;
    // Inicializar mutexes, variables de condición y barrera
    ...
    // Paso 5: Se destruiyen los mutexes, variables de condición y barrera
